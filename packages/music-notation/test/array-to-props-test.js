...
var toProps = require('../array/to-props')

function prop (i, str, parser) {
  return str.split(' ').map(parser).map(toProps).map(function (e) { return e[i] })
}

describe('music-notation/array-props', function () {
  ...
  it('invalid pitches', function () {
    assert.equal(toProps(), null)
    assert.equal(toProps(null), null)
    assert.equal(toProps('C2'), null)
    assert.equal(toProps('blah'), null)
  })
})
