...
describe("tonal-scale", () => {
  ...
  test("get", () => {
    expect(scale.get("major", "C")).toEqual(arr("C D E F G A B"));
    expect(scale.get("major", "C2")).toEqual(arr("C2 D2 E2 F2 G2 A2 B2"));
    // alias
    expect(scale.get("ionian", "C")).toEqual(arr("C D E F G A B"));
    // intervals
    expect(scale.get("major", false)).toEqual(arr("1P 2M 3M 4P 5P 6M 7M"));
    // partially applied
    expect(scale.get("major")("Db3")).toEqual(arr("Db3 Eb3 F3 Gb3 Ab3 Bb3 C4"));
    // not found
