...
describe("tonal-distance", () => {
  ...
  test("distances between pitch classes are always ascending", () => {
    ...
    expect(fromC("c d e f g a b")).toEqual([
      "1P",
      "2M",
      "3M",
      "4P",
    ...
    expect(fromG("c d e f g a b")).toEqual([
      "4P",
      "5P",
      "6M",
      "7m",
  ...
  test("get difference between intervals", () => {
    ...
    expect(subsM2("P1 M2 M3 P4 P5 M6 M7")).toEqual([
      "-2M",
      "1P",
      "2M",
      "3m",
  ...
  test("when both pitches are in arra format, return array format", () => {
    expect(dist.interval(["tnlp", [0, 0]], ["tnlp", [1, 0]])).toEqual([
      "tnlp",
      [1, 0],
      1
    ]);
