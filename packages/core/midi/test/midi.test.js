...
describe("tonal-midi", () => {
  test("toMidi - map note names to note numbers", () => {
    expect(map(midi.toMidi, "C4 D4 E4 F4 G4 A4 B4 C5")).toEqual([
      60,
      62,
      64,
      65,
  ...
  test("toMidi - pitch classes do not have midi", () => {
    expect(map(midi.toMidi, "C D E F G A B")).toEqual([
      null,
      null,
      null,
      null,
  ...
  test("note - get names from midi numbers", () => {
    expect(
      map(midi.note, [60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72])
    ).toEqual([
      "C4",
      "Db4",
      "D4",
  ...
  test("note - partially applied with true", () => {
    expect(
      map(midi.note(true), [60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72])
    ).toEqual([
      "C4",
      "C#4",
      "D4",
