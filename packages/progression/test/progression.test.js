...
describe('tonal-progression', function () {
  test('concrete', function () {
    ...
    expect(prog.concrete('I II III IV V VI VII', 'C')).toEqual([ 'C', 'D', 'E', 'F', 'G', 'A', 'B' ])
    expect(prog.concrete('bI bII bIII bIV bV bVI bVII', 'C')).toEqual([ 'Cb', 'Db', 'Eb', 'Fb', 'Gb', 'Ab', 'Bb' ])
    expect(prog.concrete('#Im7 #IIm7 #III #IVMaj7 #V7 #VI #VIIo', 'C')).toEqual([ 'C#m7', 'D#m7', 'E#', 'F#Maj7', 'G#7', 'A#', 'B#o' ])
  })
  ...
  test('progressions: build roman chord', function () {
    expect(
      [0, 1, 2, 3, 4, 5, 6, 7, 8].map(function (n) { return prog.buildRoman(n) })
    ).toEqual([ 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'I', 'II' ])
    expect(prog.buildRoman(2, -1)).toBe('bIII')
    expect(prog.buildRoman(3, 1, 'dim')).toBe('#IVdim')
  })

  test('parseRomanChord', function () {
