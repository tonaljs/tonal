...
test('tonal-spell', function () {
  ...
  test('get fifths from F', function () {
    ...
    expect('Bb Eb Ab Db Gb Cb Fb'.split(' ').map(spell.fifths))
      .toEqual([ -1, -2, -3, -4, -5, -6, -7 ])
    expect('Bbb Ebb Abb Dbb Gbb Cbb Fbb'.split(' ').map(spell.fifths))
  ...
  test('get note names', function () {
    ...
    var tr = (b) => (x) => x + b
    var scale = (type) => (root) => type.map(tr(spell.pitch(root) + 60)).map(spell.pcIn(root)).join(' ')
    expect(notes.map(scale(major))).toEqual([
      'C D E F G A B', 'D E F# G A B C#', 'E F# G# A B C# D#', 'F G A Bb C D E',
      'G A B C D E F#', 'A B C# D E F# G#', 'B C# D# E F# G# A#'
    ])
