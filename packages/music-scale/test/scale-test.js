...
describe('music-scale', function () {
  describe('create a scale', function () {
    it('from notes', function () {
      assert.deepEqual(scale('C2 C4 C C6', null), ['C'])
      assert.deepEqual(scale('f c g c6 e5', null), ['F', 'G', 'C', 'E'])
    })
    it('from intervals', function () {
      assert.deepEqual(scale('1 2 3 8 9 10 11', null), [ '1P', '2M', '3M', '4P' ])
      assert.deepEqual(scale('11 10 9', null), [ '2M', '3M', '4P' ])
    })
  })
