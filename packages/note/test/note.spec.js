...
describe("tonal-note", function() {
  ...
  test("midi", function() {
    var midis = "c4 d4 e4 f4 g4 a4 b4 c4 c-1 c-2".split(" ").map(note.midi);
  ...
  test("pc", function() {
    var pcs = map(note.pc, "a b0 d2 e# fb3 g###4 bbbb5");
    expect(pcs).toEqual(["A", "B", "D", "E#", "Fb", "G###", "Bbbb"]);
    expect(note.pc("blah")).toBe(null);
    expect(note.pc("h")).toBe(null);
  });
  test("altToAcc", function() {
    var accs = [-4, -3, -2, -1, 0, 1, 2, 3, 4].map(note.altToAcc);
    var expected = ["bbbb", "bbb", "bb", "b", "", "#", "##", "###", "####"];
    expect(accs).toEqual(expected);
  });
  test("stepToLetter", function() {
  ...
  test("simplify", function() {
    var notes = $("C## C### F##4 Gbbb5 B#4 Cbb4");
    expect(notes.map(note.simplify)).toEqual($("D D# G4 E5 C5 Bb3"));
    expect(
      notes.map(function(n) {
        return note.simplify(n, false);
      })
