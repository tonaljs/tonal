...
describe("distance", () => {
  describe("find intervals between notes", () => {
    test("interval between notes", () => {
      var fromC3 = lift(Distance.interval("C3"));
      expect(fromC3("C3 e3 e4 c2 e2")).toEqual("1P 3M 10M -8P -6m");
    });

    test("intervals between pitch classes are always ascending", () => {
      expect(Distance.interval("C", "D")).toEqual("2M");

      var fromC = lift(Distance.interval("C"));
      expect(fromC("c d e f g a b")).toEqual("1P 2M 3M 4P 5P 6M 7M");

      var fromG = lift(Distance.interval("G"));
      expect(fromG("c d e f g a b")).toEqual("4P 5P 6M 7m 1P 2M 3M");
    });
    test("note types can not be mixed", () => {
      expect(Distance.interval("C", "C2")).toBe(null);
      expect(Distance.interval("C2", "C")).toBe(null);
    });
  });

  test("semitones", () => {
    expect(Distance.semitones("C3", "G#3")).toBe(8);
    expect(Distance.semitones("C4", "A3")).toBe(-3);

    const fromC = lift(Distance.semitones("C"));
    expect(fromC("C D E F G A B")).toEqual("0 2 4 5 7 9 11");

    const fromB = lift(Distance.semitones("B"));
    expect(fromB("C D E F G A B")).toEqual("1 3 5 6 8 10 0");

    expect(Distance.semitones("blah", "C3")).toBe(null);
  });

  describe("transposition", () => {
    test("transpose pitch classes by intervals", () => {
      const trFromBb = lift(Distance.transpose("Bb"));
      expect(trFromBb("P1 M3 P5 M7")).toEqual("Bb D F A");
    });

    test("transpose notes by intervals", () => {
      const trFromBb2 = lift(Distance.transpose("Bb2"));
    ...
    test("transpose by interval", () => {
      const trBy3M = lift(Distance.transposeBy("3M"));
    ...
    test("transpose by descending intervals", () => {
      const trDescM2 = lift(Distance.transposeBy("-2M"));
      expect(trDescM2("c2 d3 f4 g5")).toEqual("Bb1 C3 Eb4 F5");
    });

    test("transpose edge cases", () => {
      const fromC2 = lift(Distance.transpose("C2"));
  ...
  describe("transpose intervals", () => {
    ...
    test("subtract intervals", () => {
      expect(Distance.subtract("5P", "3M")).toEqual("3m");
      expect(Distance.subtract("3M", "5P")).toEqual("-3m");
    });
  });
  ...
  describe("fifths", () => {
    test("transpose fifths", () => {
      expect(
        [0, 1, 2, 3, 4, 5, 6, 7].map(Distance.trFifths("C")).join(" ")
      ).toEqual("C G D A E B F# C#");
      expect(
        [0, 1, 2, 3, 4, 5, 6, 7].map(Distance.trFifths("C4")).join(" ")
      ).toEqual("C G D A E B F# C#");
    });

    test("distance in fifths", () => {
      expect(Distance.fifths("C", "G")).toBe(1);
      expect(Distance.fifths("G", "D")).toBe(1);
      expect(Distance.fifths("C", "C#")).toBe(7);
    });
  });
});
