...
vows.describe('pitch/transpose').addBatch({
  'transpose edge cases': function () {
    assert.equal(transpose('Gb', 'P4'), 'Cb5')
    assert.equal(transpose('F4', 'P-5'), 'Bb3')
    assert.equal(transpose('E4', 'M2'), 'F#4')
    assert.equal(transpose('F4', 'M-2'), 'Eb4')
  },
  ...
  'transpose notes': function () {
    ...
    assert.deepEqual(notes.map(transpose.by('M9')),
      ['D5', 'E5', 'F#5', 'G5', 'A5', 'B5', 'C#6'])
    assert.deepEqual(notes.map(transpose.by('M-9')),
