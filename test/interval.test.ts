import { tokenize, interval } from '../src/interval'
...
describe('interval', () => {
  ...
  describe('interval from string', () => {
    ...
    test('name', () => {
      const names = lift(i => interval(i).name)
      ...
      expect(interval('not-an-interval').name).toBe(undefined)
      expect(interval('2P').name).toBe(undefined)
    })
    test('q', () => {
      const q = lift(i => interval(i).q)
      expect(q('1dd 1d 1P 1A 1AA')).toEqual('dd d P A AA')
      expect(q('2dd 2d 2m 2M 2A 2AA')).toEqual('dd d m M A AA')
    })

    test('alt', () => {
      const alt = lift(i => '' + interval(i).alt)
      expect(alt('1dd 2dd 3dd 4dd')).toEqual('-2 -3 -3 -2')
    })

    test('simple', () => {
      const simple = lift(i => '' + interval(i).simple)
  ...
  describe('interval from pitch props', () => {
    ...
    it('returns undefined if dir is not present', () => {
      expect(interval({ step: 0, alt: 0, oct: 0 }).name).toBe(undefined)
    })
  })
})
