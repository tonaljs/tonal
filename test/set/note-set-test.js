var vows = require('vows')
var assert = require('assert')
var notes = require('../../lib/set/note-set')

vows.describe('Set').addBatch({
  'note set from decimal': function () {
    assert.deepEqual(notes(2773, 'C'), ['C4', 'D4', 'E4', 'F4', 'G4', 'A4', 'B4'])
  },
  'note set from binary': function () {
    assert.deepEqual(notes('11', 'C'), ['C4', 'Db4'])
    assert.deepEqual(notes('101', 'C'), ['C4', 'D4'])
    assert.deepEqual(notes('100000000001', 'C'), ['C4', 'B4'])
    assert.deepEqual(notes('1000000000001', 'C'), ['C4', 'C5'])
  },
  'note set from interval set': function () {
    assert.deepEqual(notes(['P1', 'm3'], 'C'), ['C4', 'Eb4'])
  },
  'note set from note set': function () {
    assert.deepEqual(notes(['C', 'D', 'E', 'F', 'G', 'A', 'B'], 'C'),
      ['C', 'D', 'E', 'F', 'G', 'A', 'B'])
    assert.deepEqual(notes(['C', 'D', 'E', 'F', 'G', 'A', 'B'], 'D'),
      ['D4', 'E4', 'F#4', 'G4', 'A4', 'B4', 'C#5'])
  }
}).export(module)
