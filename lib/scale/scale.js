...
var harmonizer = require('../interval/harmonizer')
...
function scale (tonic, name) {
  ...
  var intervals = scales[name]
  if (intervals) return harmonizer(intervals)(tonic, true)
  var m = IMPLICIT.exec(name)
  return m && scales[m[2]] ? scale(m[1], m[2]) : null
}
