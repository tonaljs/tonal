...
/**
 * Get the relation between a pitch and the number of steps in the
 * cycle of fifths (with root in C)
 *
 * @param {Integer|String} step - if it's an integer, returns the pitch step after
 * moving `step` steps in the cycle. If it's a step string, returns the number
 * of steps starting from 'C' to the given step
 * @return {String|Integer} - the pitch name or the number of steps (depending of the param)
 *
 * @example
