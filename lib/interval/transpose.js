var parseNote = require('../note/parseStrict')
var accidentals = require('../note/accidentals')
var step = require('../note/step')
var parseInterval = require('./parseStrict')
...
function transpose (interval, note) {
  ...
  if (interval.generic === 0 && interval.quality === 'P') {
    return note.step + note.acc + (note.oct + interval.dir * interval.oct)
  }
  ...
  if (interval.dir === -1) {
    ...
    interSize = semitones(interval, true) - 12
  } else {
    interSize = semitones(interval, true)
  }
  ...
  var destSize = (SEMITONES[destStep] - note.pc)
  ...
  // return the note
  return destStep + accidentals(difference) + oct
}
