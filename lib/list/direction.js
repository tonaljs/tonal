...
function direction (list, dir) {
  ...
  var dirFn = list[0] === 'P1' ? intervalDirection : notesDirection
  return dirFn(list, comp, off)
}

function intervalDirection (list, comp, off) {
  var prev, curr
  var result = list.slice(0, 1)
  for (var i = 1; i < list.length; i++) {
    prev = interval(result[i - 1])
    curr = interval(list[i])
    if (comp(prev.semitones, curr.semitones)) {
      var oct = Math.floor((prev.semitones - curr.semitones) / 12) + off
      result.push(interval(curr, null, curr.oct + oct).name)
    } else {
      result.push(list[i])
    }
  }
...
function notesDirection (list, comp, off) {
  var prev, curr
  var result = list.slice(0, 1)
  for (var i = 1; i < list.length; i++) {
