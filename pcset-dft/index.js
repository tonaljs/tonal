 ...
 * @module PcsetDft
 */
export default { dft, spectra, distance };

const { PI, sin, cos, pow, sqrt } = Math;

function pcset(notes) {
  return Object.keys(
    notes.map(chroma).reduce(function(set, ch) {
      set[ch] = true;
      return set;
    }, {})
  );
 ...
 *
 * @param {Array<string>} notes - the notes or pitch class set
 * @return {Array<number, number>} the fourier components
 *
 * @example
 ...
 *
 * @param {Array<string>} notes - the notes or pitch set
 ...
 * @return number - the Euclidean distance between both
 *
 * @example
 * PcsetDft.distance(["C", "E", "G"], ["C", "Eb", "G"])) //=> 1.5307337294603596
 */
export function distance(set1, set2) {
  const sp1 = spectra(set1);
  const sp2 = spectra(set2);
  return Math.sqrt(
