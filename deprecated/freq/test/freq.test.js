...
describe("tonal-freq", () => {
  ...
  it("freq: eqTempFreq", () => {
    ...
    expect(
      "c4 d4 e4 f4 g4 a4 b4".split(" ").map(freq.eqTempFreq(440, 1))
    ).toEqual([261.6, 293.7, 329.6, 349.2, 392, 440, 493.9]);
    expect(
      "c4 d4 e4 f4 g4 a4 b4".split(" ").map(freq.eqTempFreq(440, 2))
    ).toEqual([261.63, 293.66, 329.63, 349.23, 392, 440, 493.88]);
  });
});
