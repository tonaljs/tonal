...
describe("tonal-pitch", () => {
  ...
  test("note to string", () => {
    function id(n) {
      return p.strNote(p.parseNote(n));
    }
    expect("a c db2 e#4 fx6 gbbb ab#9".split(" ").map(id)).toEqual([
      "A",
      "C",
      "Db2",
      "E#4",
  ...
  test("interval to string", () => {
    function id(i) {
      return p.strIvl(p.parseIvl(i));
    }
    expect("1P 2M 3M 4P 5P 6M 7M".split(" ").map(id)).toEqual([
      "1P",
      "2M",
      "3M",
      "4P",
    ...
    expect("1d 2m 3m 4d 5d 6m 7m".split(" ").map(id)).toEqual([
      "1d",
      "2m",
      "3m",
      "4d",
    ...
    expect("8A 9A 10A 11A 12A 13A 14A".split(" ").map(id)).toEqual([
      "8A",
      "9A",
      "10A",
      "11A",
    ...
    expect("-8d -9m -10m -11d -12d -13m -14m".split(" ").map(id)).toEqual([
      "-8d",
      "-9m",
      "-10m",
      "-11d",
