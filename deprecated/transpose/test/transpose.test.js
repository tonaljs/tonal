...
describe("tonal-scale", () => {
  ...
  test("notes by intervals", () => {
    expect(map(tr("3M"), "c2 d3 f4 g5")).toEqual(["E2", "F#3", "A4", "B5"]);
  });
  test("pitch classes by intervals", () => {
    expect(map(tr("Bb"), "P1 M3 P5 M7")).toEqual(["Bb", "D", "F", "A"]);
  });
  ...
  test("notes by descending intervals", () => {
    expect(map(tr("-2M"), "c2 d3 f4 g5")).toEqual(["Bb1", "C3", "Eb4", "F5"]);
  });
  test("intervals by intervals", () => {
    expect(map(tr("3M"), "1P 2M 3M 4P 5P")).toEqual([
      "3M",
      "4A",
      "5A",
      "6M",
  ...
  test("descending intervals", () => {
    expect(map(tr("-2M"), "1P 2M 3M 4P 5P")).toEqual([
      "-2M",
      "1P",
      "2M",
      "3m",
  ...
  test("all desending intervals", () => {
    expect(map(tr("-2M"), "-5P -4P -3M -2M 1P")).toEqual([
      "-6M",
      "-5P",
      "-4A",
      "-3M",
  ...
  test("returns array notation if both params are in array notation", () => {
    expect(tr(["tnlp", [1, 0], 1], ["tnlp", [1, 0]])).toEqual(["tnlp", [2, 0]]);
  });
  test("transpose edge cases", () => {
    var trC = function(i) {
      return i.split(" ").map(tr("C2"));
    };
    ...
    expect(trC("4dd 4d 4P 4A 4AA")).toEqual([
      "Fbb2",
      "Fb2",
      "F2",
      "F#2",
  ...
  test("transpose fifths", () => {
    expect([0, 1, 2, 3, 4, 5, 6, 7].map(t.trFifths("C"))).toEqual([
      "C",
      "G",
      "D",
      "A",
